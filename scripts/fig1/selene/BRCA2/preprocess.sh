name='GSE133450_BRCA2'
size="2000"

#bgzip -d wgEncodeAwgTfbsUtaGm12878CtcfUniPk.narrowPeak.gz
Rscript --vanilla get_fasta.R  GSE133450_BRCA2.anno.txt ${name} ${size}
...
sed -i "s/$/\tBRCA2/" hg19_blacklisted/$name.cut.bed
echo "sed ok"
...
sed -i "s/$/\tBRCA2/" hg19_blacklisted_400bestAsiSI_masked/$name.cut.bed
echo "sed ok"
