...
Rscript --vanilla get_fasta.R  HVL7VBGX5_Lig4_2_DIVA_sequence.rmdups_q0.1_peaks.narrowPeak ${name} ${size}
...
sed -i "s/$/\tlig4/" hg19_blacklisted/$name.cut.bed
echo "sed ok"
...
sed -i "s/$/\tlig4/" hg19_blacklisted_400bestAsiSI_masked/$name.cut.bed
echo "sed ok"
