#Sébastien AUBER
#06/08/2025
#THis script Allows to make correlation heatmap
library(dplyr)
library(tidyr)
library(corrplot)
library(matsbyname)
...
if(DSB==T){  #restrain to the SNPs that are dsb SNPs
  if(expe_type=="ChIP-seq"){
...
###################Plot dsbSNP vs all protein SNP
##get only dsbSNP on one column
ordered_corinfo_dsb<-sort_rows_cols( cor.info3,roworder = row.names(cor.info3)[clust$order], colorder = colnames(cor.info3)[clust$order])
ordered_corinfo_dsb<-ordered_corinfo3%>%as_tibble%>%dplyr::select(pATM,SETX,RPA,X53BP1,Lig4)%>%as.matrix()
row.names(ordered_corinfo_dsb)<-row.names(ordered_corinfo3)
#plot and save plot
pdf("warD_pearson_dsb.pdf",11,8)

p<-corrplot(t(ordered_corinfo_dsb),  order="original", tl.cex=1.3, cl.cex=1, tl.col="black", addrect=6,is.corr = T,col = COL2('RdBu', 8),
         na.label = "square", na.label.col = "grey")#+#,col.lim = c(-0.5,0.5))
dev.off()
...
get_hex<-function(request){ #function to create hex bin
