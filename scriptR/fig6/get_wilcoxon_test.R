############################################################################
#this script allows to compute Wilcoxon test between interest SNP types and
...
for (SNP_type in SNP_type_list) {
  ...
  for (ctrl_type in ctrl_list) {
    ...
    #perform Wilcoxon exact test using several inclusion thresholds
    ...
    for (thresP in c(
            ...
            if (!("harmonised file has a 0 size" %in% listPval[[i]]$p_interest)) {
              ...
              if (length(listPvali$p_interest < thresP) > 10) {
                if (length(listPvali$p_ctrl < thresP) > 10) {
                  ...
                  matMedPval[i,3]=wilcox.test(listPvali$p_dsb[listPvali$p_dsb<thresP],listPvali$p_ctrl[listPvali$p_ctrl<thresP],
      ...
      dir.create("Analysis_pval_TAG2")
      dir.create(paste0("Analysis_pval_TAG2/", ctrl_type))
      dir.create(paste0("Analysis_pval_TAG2/", ctrl_type, "/thresP"))
      write.csv(
        matMedPval,
        paste0(
          "Analysis_pval_TAG2/",
      ...
      #correct Wilcoxon p_values (using fdr)
...
write.table(matCorrected,paste0("Analysis_pval_TAG2/",ctrl_type,"/thresP","/matMedPvalcorrected_sup1000_harmonised_filtered_with_fdr_6000SNP_",ctrl_type,"_thresP_",thresP,"_TAG.tsv"),sep="\t",row.names = F)
